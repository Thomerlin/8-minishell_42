...
/*   Updated: 2022/05/31 00:47:31 by tyago-ri         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	main(int argc, char *argv[], char *envp[])
{
	...
	g_data->list.env = copy_first_env(envp);
	printf("%s",*g_data->list.env); //Copiando o Path Env
	if (argc == 1 && argv[0] != NULL)
	{
		...
		while (1)
		{
			...
			if (data.line[0] != '\0')
			{
				...
				data.line = (t_mns *) malloc (sizeof (t_mns)); // tratar erro, linha 182
